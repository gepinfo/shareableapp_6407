...
public GpDelete(req: Request, res: Response) {
        ...
        new ApiAdapter().delete(Constant.NEWAPPURL + `${req.url}` )
...
public GpGetAllValues(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.NEWAPPURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.NEWAPPURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
...
public GpUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.NEWAPPURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
    ...
    public GpSearchRelationship(req: Request, res: Response) 
                                            {
                                                ...
                                                new CustomLogger().showLogger('info', 'Enter into ticketsController.ts: GpSearch');
                                                new ApiAdapter().get(Constant.NEWAPPURL + `${req.url}`).then((res: any) => res.response.json()).then(result => 
                                                {
                                                  response1 = result;
                                                  new CustomLogger().showLogger('info', 'Exit from ticketsController.ts: GpSearch');
